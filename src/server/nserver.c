...
static int Receive_play(int ind) {
	...
	//else
	{
		...
		for (i = 0; i < TV_MAX; i++) {
			...
			/* 4.7.4 and newer clients use 32bit character size. */
			if (is_atleast(&connp->version, 4, 7, 4, 0, 0, 0))
	...
	for (x = 0; x < MAX_WINDOW_WID; x++) {
		...
		if (n >= 2) {
			...
			if (Ind2) {
				...
				/* 4.7.4 and newer clients use 32bit character size. */
				if (is_atleast(&Conn[p_ptr2->conn]->version, 4, 7, 4, 0, 0, 0)) {
...
int Send_mini_map_pos(int Ind, int x, int y, byte a, char32_t c) {
	...
	/* Packet header */
	/* 4.7.4 and newer clients use 32bit character size. */
	if (is_atleast(&p_ptr->version, 4, 7, 4, 0, 0, 0)) Packet_printf(&connp->c, "%c%hd%hd%c%u", PKT_MINI_MAP_POS, xs, ys, a, c);
