...
again:
		...
		// Prefix stays constant ("F4K"), suffix derived from a local SDL2-friendly fingerprint.
		{
			...
			c_msg_print("Generating deterministic fake MAC address'\n");
			MD5Init(&ctx);
			...
			fprintf(stderr, "jezek - Generated deterministic fake MAC address: %x%x%x%x%x%x\n", ip_iaddr[0], ip_iaddr[1], ip_iaddr[2], ip_iaddr[3], ip_iaddr[4], ip_iaddr[5]);
		}
#endif
	}
