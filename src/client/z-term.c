...
static void Term_fresh_row_both_wipe(int y) {
	...
	for (x = x1; x <= x2; x++) {
		...
		if (nc > MAX_FONT_CHAR) {
			...
			/* flick attribute color */
#ifdef EXTENDED_COLOURS_PALANIM
			if (na >= TERMA_DARK && na <= TERMA_L_UMBER) na = na - TERMA_OFFSET + 16; /* Use 'real' extended terminal colours ie 16..31 */
			else
#endif
#ifdef EXTENDED_BG_COLOURS
...
errr Term_fresh(void) {
	...
	if (Term->soft_cursor) {
		...
		if (okay) {
			...
			/* flick attribute color */
#ifdef EXTENDED_COLOURS_PALANIM
			if (a >= TERMA_DARK && a <= TERMA_L_UMBER) a = a - TERMA_OFFSET + 16; /* Use 'real' extended terminal colours ie 16..31 */
			else
#endif
#ifdef EXTENDED_BG_COLOURS
			if (a >= TERM_MULTI && a != TERM2_BLUE)
#else
			if (a >= TERM_MULTI)
#endif
				a = flick_colour(a);
