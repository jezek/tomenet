...
bool graphics_image_prefs_has_masks = false;
/* If there is an color set for outline in pref file. */
bool graphics_image_prefs_has_outline = false;
...
void sdl2_graphics_pref_file_processed() {
	...
	if (use_graphics && graphics_reinitialize) {
		...
		/* Figure out if prefs contains masks colors setters. */
		for (uint8_t i = 0; i < graphics_image_mpt; ++i) {
			if (graphics_image_masks_colors[i] != 0) {
				graphics_image_prefs_has_masks = TRUE;
				break;
			}
		}
		fprintf(stderr, "jezek - graphics_image_prefs_has_masks: %b\n", graphics_image_prefs_has_masks);

		/* When prefs contains no masks setters, fill default colors. */
		/* Note: We assume prefs file contains all masks colors definition. If some is missing, it is on purpose. */
		if (!graphics_image_prefs_has_masks) {
			...
			graphics_image_prefs_has_outline = graphics_image_masks_colors[2] != 0;
		}
