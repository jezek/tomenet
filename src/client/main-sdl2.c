...
/*
 * Transform an SDL_Keycode to its shifted ASCII equivalent (US-ISO layout).
 * If the code is not ASCII-printable or has no shifted form, return it unchanged.
 */
static SDL_Keycode shift_keycode(SDL_Keycode keycode) {
	...
	switch (keycode) {
		...
		case '`': return('~');
		default:
			/* No shifted variant (or already shifted). */
			return(keycode);
	}
}
...
static void react_keypress(SDL_Event *event) {
	...
	if (ms) {
		...
		keycode = shift_keycode(keycode);
		fprintf(stderr, "jezek - react_keypress: after shift keycode: %d\n", keycode);
	}

	bool is_special_ascii = (keycode == SDLK_DELETE) ||
		(keycode == SDLK_BACKSPACE) ||
		(keycode == SDLK_ESCAPE) ||
		(keycode == SDLK_RETURN) ||
		(keycode == SDLK_TAB);

	if (!mc && !mo && keycode < 256 && !is_special_ascii) {
		/* No Ctrl or Alt modifier keys are pressed: send the keycode directly. */
		/* The keycode is already shifted if Shift is pressed. */
	...
	switch (keycode) {
		...
		case SDLK_DELETE:
			...
			Term_keypress('\177'); /* DEL (127, 0x7F) */
			return;
...
void sdl2_graphics_pref_file_processed() {
	...
	if (use_graphics && graphics_reinitialize) {
		fprintf(stderr, "jezek - graphics_image_prefs_has_masks: %b\n", graphics_image_prefs_has_masks);
		if (!graphics_image_prefs_has_masks) {
			fprintf(stderr, "Warning: Masks colors are not set in pref file. Using default values\n");
		}
		fprintf(stderr, "jezek - graphics_image_prefs_has_outline: %b\n", graphics_image_prefs_has_outline);
...
/*
 * Scale source surface `src` containing tiles with width `src_tile_wdt`
 * and height `src_tile_hgt` pixels.
 * The returned surface contains scaled tiles sized to `dst_tile_wdt` and
 * `dst_tile_hgt`, and keeps the source surface format.
 * Assumes source width/height are divisible by source tile width/height
 * and that a surface pixel is 32-bit wide.
 */
SDL_Surface *ScaleSurface(SDL_Surface *src, uint16_t src_tile_wdt, uint16_t src_tile_hgt, uint16_t dst_tile_wdt, uint16_t dst_tile_hgt) {
	...
	/* Count tiles horizontally/vertically and resulting scaled dimensions. */
		...
		for (int y = 0; y < layers[0]->h; y++) {
			for (int x = 0; x < layers[0]->w; x++) {
				...
				for (int l = 1; l < nlayers; l++) {
					...
					if (srcPixels[pos] == SDL_MapRGBA(layers[0]->format, ((mask_colors[l + 1] >> 24) & 0xFF), ((mask_colors[l + 1] >> 16) & 0xFF), ((mask_colors[l + 1] >> 8) & 0xFF), 0xFF)) {
						uint32_t *dstPixels = (uint32_t*)layers[l]->pixels;
						dstPixels[pos] = srcPixels[pos];
						srcPixels[pos] = SDL_MapRGBA(layers[0]->format, ((mask_colors[0] >> 24) & 0xFF) ^ 0xFF, ((mask_colors[0] >> 16) & 0xFF) ^ 0xFF, ((mask_colors[0] >> 8) & 0xFF) ^ 0xFF, 0);
					}
				}
			}
		}
...
static void generate_outline(SDL_Surface *scaled_image, SDL_Surface **layers, uint8_t nlayers, int tile_w, int tile_h, const uint32_t mask_colors[]) {
	...
	uint32_t bg_mask_color = SDL_MapRGBA(scaled_image->format, ((mask_colors[0] >> 24) & 0xFF), ((mask_colors[0] >> 16) & 0xFF), ((mask_colors[0] >> 8) & 0xFF), 0xFF);
	uint32_t ol_mask_color = SDL_MapRGBA(scaled_image->format, ((mask_colors[ol+1] >> 24) & 0xFF), ((mask_colors[ol+1] >> 16) & 0xFF), ((mask_colors[ol+1] >> 8) & 0xFF), 0xFF);
		...
		for (int col = 0; col < tiles_per_row; col++) {
			...
			for (int x = x_start; x < x_end; x++) {
				...
				while (right < bound) {
					...
					if (mask_colors[ol+1] != 0 && srcPixels[pos+right] == ol_mask_color) found_bg = TRUE;
					if (!found_bg) {
						cnt++;
					}
				}
				...
				while (left < bound) {
					...
					if (mask_colors[ol+1] != 0 && srcPixels[pos+left] == ol_mask_color) found_bg = TRUE;
					if (!found_bg) {
						cnt--;
					}
					left++;
	...
	for (int y = 0; y < scaled_image->h; y++) {
		for (int x = 0; x < scaled_image->w; x++) {
			...
			if (mask_colors[ol+1] != 0 && srcPixels[pos] == ol_mask_color) found_bg = TRUE;
			if(vpass[pos] == 1 && found_bg) {
				uint32_t* dstPixels = (uint32_t*)layers[ol]->pixels;
				dstPixels[pos] = SDL_MapRGBA(layers[ol]->format, ((mask_colors[ol+1] >> 24) & 0xFF), ((mask_colors[ol+1] >> 16) & 0xFF), ((mask_colors[ol+1] >> 8) & 0xFF), 0xFF);
			}
		}
	}
...
static void clear_outline_mask_single_layer(uint8_t nlayers, SDL_Surface **layers, bool has_outline, const uint32_t mask_colors[]) {
	...
	for (int y = 0; y < layers[0]->h; y++) {
		for (int x = 0; x < layers[0]->w; x++) {
			uint32_t pos = (y * layers[0]->w) + x;
			if (srcPixels[pos] == SDL_MapRGBA(layers[0]->format, ((mask_colors[2] >> 24) & 0xFF), ((mask_colors[2] >> 16) & 0xFF), ((mask_colors[2] >> 8) & 0xFF), 0xFF)) {
				srcPixels[pos] = SDL_MapRGBA(layers[0]->format, ((mask_colors[0] >> 24) & 0xFF) ^ 0xFF, ((mask_colors[0] >> 16) & 0xFF) ^ 0xFF, ((mask_colors[0] >> 8) & 0xFF) ^ 0xFF, 0);
			}
		}
	}
...
static errr term_data_init(int index, term_data *td, bool fixed, cptr name, cptr font) {
	...
	if (use_graphics) {
		...
		fprintf(stderr, "jezek - term_data_init: gfx initialization for term %d\n", index);
		sdl2_ensure_graphics_mask_colors();
		term_data_init_graphics(td);
